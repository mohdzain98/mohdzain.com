    ...
    <div id='repos' style={{padding:'2%'}}>
      <div className='container p-4' style={{marginTop:'1%'}}>
            <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Github Repositories
                </li>
            </ol>
        ...
        {repo.map((item)=>{
            return(
                ...
                <div class='dis' onClick={() => handleClick(item.url)}>
                    <div class='data'>
                    <Link to={item.url} target='_blank' rel='noopener' style={{textDecoration:'none'}}>{item.name}</Link>
                    <p class="define">{item.define}</p>
                    <p class='visible top-right'>{item.visible}</p>
                    <p class='top-down'>
                        <i class='fa fa-circle' style={{color:`${item.color}`,marginRight:'4px'}}></i>{item.lang}
                    </p>
                    </div>
                </div>
                </AnimatedOnScroll>
